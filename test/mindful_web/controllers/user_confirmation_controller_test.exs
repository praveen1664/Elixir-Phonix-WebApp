defmodule MindfulWeb.UserConfirmationControllerTest do
  ...
  describe "GET /users/confirm" do
    test "renders the resend confirmation page", %{conn: conn} do
      ...
      assert response =~ "Send confirmation instructions"
    end
  end
  ...
  describe "POST /users/confirm/:token" do
    test "confirms the given token once", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == "/"
      assert get_flash(conn, :success) =~ "Your email has been confirmed."
      ...
      conn = post(conn, Routes.user_confirmation_path(conn, :update, token))
      assert redirected_to(conn) == "/"
      assert get_flash(conn, :error) =~ "Your confirmation link is invalid or it has expired"
